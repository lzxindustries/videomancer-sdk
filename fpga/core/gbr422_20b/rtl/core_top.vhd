...
begin
  ...
  -- Data still arrives through the DDR RGB unpack, but sync comes from the
  -- free-running sync generator rather than the decoder's HS/VS/FIELD-DE
  -- pins, which carry nothing dependable when the CP has no locked input.
  -- Feeding a dead DE into AVID blanks the whole frame, since AVID gates
  -- both program output and the blanking stage.
  --
  ...
  GEN_SD_STANDALONE_IN : if C_ENABLE_SD and C_ENABLE_STANDALONE generate
    ...
    signal s_ddr_gbr422 : t_video_stream_gbr422_20b;
  begin
    ...
    s_video_in.g <= s_ddr_gbr422.g;
    s_video_in.c <= s_ddr_gbr422.c;
