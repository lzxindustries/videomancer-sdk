...
begin
  ...
  -- rate (27 MHz for 13.5 MHz NTSC/PAL pixels) and at 1x for HD.
  -- ADV7513 HDMI transmitter is clocked at the pixel rate (vid_clk).
  -- In SD standalone, LLC is already 27 MHz so we feed it directly to
  -- the encoder without an oversampling PLL. In HD standalone, vid_clk
  -- is already 74.25 MHz, so it drives both the encoder and HDMI tx.
