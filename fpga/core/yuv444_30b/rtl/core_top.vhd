...
begin
  ...
  GEN_SD_STANDALONE_IN : if C_ENABLE_SD and C_ENABLE_STANDALONE generate
    ...
    -- LLC always arrives at 27 MHz from the ADV7181C; 13.5 MHz is
    -- generated by dividing LLC by 2 in fabric. A glitch-free 2:1 clock
    -- mux selects the active rate. Both reference clocks are exposed
    -- at architecture scope so GEN_SD_STANDALONE_OUT can drive its
    -- encoder-clock PLL from the 13.5 MHz reference (matching the
    -- working SD HDMI/Analog OUT pattern).
  begin
  ...
  begin
    -- PLL ×2 from the 13.5 MHz LLC÷2 reference. For NTSC/PAL this
    -- produces a 27 MHz encoder clock phase-aligned to vid_clk. The
    -- PLL runs continuously regardless of timing-id selection (it is
    -- always fed from the 13.5 MHz divider, not from vid_clk).
    pll_inst : entity work.sd_video_clk_pll_2x
    port map(
