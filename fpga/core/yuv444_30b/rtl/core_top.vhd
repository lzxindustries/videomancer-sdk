...
begin
  ...
  GEN_SD_STANDALONE_IN : if C_ENABLE_SD and C_ENABLE_STANDALONE generate
    ...
    -- ADV7181C in clock-generator-only mode. Clock generation is:
    --
    --   i_vid_dec_clk (27 MHz LLC pad)
    --     |--> fabric ÷2 register --> 13.5 MHz fabric --> sd_video_clk_pll_2x
    --                                                       |
    --                                                       v
    --                                          s_sd_standalone_clk_27_pll
    --                                                  (clean global 27 MHz)
    --                                                       |
    --                                                       +--> not --> o_vid_enc_clk
    --                                                       +--> not --> o_hdmi_tx_clk
    --                                                       |
    --                                                       v
    --                                          fabric ÷2 register --> vid_clk (13.5)
    --
    -- vid_clk and the encoder/HDMI CLK are now both derived from the
    -- same PLL output, deterministically phased. ADV7393 sees data
    -- launched on vid_clk's rising edge and samples on the falling
    -- edge of the 27 MHz CLK (mid-data-cycle). ADV7513 sees a clean
    -- 27 MHz from the PLL global net, so its TMDS PLL can lock.
    signal s_llc_div2_pre  : std_logic := '0';
    signal s_vid_clk_div2  : std_logic := '0';
  begin
    -- Stage 1: divide raw LLC pad by 2 in fabric to feed the PLL.
    ...
    -- Stage 2: clean 13.5 MHz → 27 MHz on a global clock net.
    pll_inst : entity work.sd_video_clk_pll_2x
    port map(
    ...
    -- Stage 3: divide PLL output by 2 to make vid_clk (13.5 MHz),
    -- now phase-aligned with the encoder/HDMI 27 MHz output clocks.
    ...
    -- Decoder data path disconnected: drive black pixels into the
    -- pipeline, gate active video by the free-running internal sync
    -- generator. Sync references tied inactive so video_sync_generator
    -- counter free-runs without phantom resets from output self-feedback.
  ...
  begin
    -- HD standalone: LLC is already the correct 74.25 MHz (or /1.001)
    -- pixel clock; pass through unmodified. All 11 HD timings (including
    -- dropframe rates) are supported because firmware programs the
    -- CP-PLL divisor to track the selected timing exactly.
    vid_clk <= i_vid_dec_clk;
    ...
    s_video_in.avid    <= s_o_avid;
    s_video_in.field_n <= '1';
    -- Sync references tied inactive so video_sync_generator counter
    -- free-runs without phantom resets from output self-feedback.
    s_sync_ref_hsync_n <= '1';
    s_sync_ref_vsync_n <= '1';
