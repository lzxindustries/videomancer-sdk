...
entity core_top is
  port(
    ...
    -- Pin 128 split into two ports. In non-standalone bitstreams the
    -- FPGA drives `o_mcu_gpout_clk` as a debug heartbeat; the input is
    -- ignored. In standalone bitstreams the FPGA reads `i_mcu_ref_clk`
    -- as a 27 MHz pixel clock reference from the RP2040 GPOUT
    -- peripheral; the output driver is disabled by hardware_top via SB_IO.
...
architecture rtl of core_top is
  ...
  signal s_prog_data_out : t_video_stream_yuv444_30b;
...
begin
  ...
  GEN_SD_DUAL_IN : if C_ENABLE_SD and C_ENABLE_DUAL generate
    ...
    s_video_in.c(9 downto 0) <= i_vid_dec_d(19 downto 10);
    -- Sync and active-video gating come from the internal free-running
    -- video sync generator. Reference selection (sync_ref_*) is handled
    -- by the dedicated CDC block below.
    s_video_in.hsync_n <= s_o_hsync;
    s_video_in.vsync_n <= s_o_vsync;
    s_video_in.avid    <= s_o_avid;
    s_video_in.field_n <= '1';
  ...
  -- ========================================================================
  -- In standalone bitstreams the analog decoder and HDMI receiver are
  -- physically disconnected from the video pipeline and the FPGA generates
  -- all pixel clocks internally from a 27 MHz reference clock supplied by
  -- the RP2040 GPOUT peripheral on pin 128 (RP2040_GPOUT_CLK). The output
  -- driver on that pin is tri-stated so it can be read as an input.

  GEN_SD_STANDALONE_IN : if C_ENABLE_SD and C_ENABLE_STANDALONE generate
    -- SD standalone supports two pixel rates selected at runtime by
    -- s_video_timing_id bit 2:
    --   bit 2 = 0 -> 13.5 MHz (NTSC=0x0, PAL=0x8)
    --   bit 2 = 1 -> 27   MHz (480p=0x4, 576p=0xC)
    -- 13.5 MHz is generated by dividing the 27 MHz reference by 2 in
    -- fabric. A glitch-free 2:1 clock mux selects the active rate.
    signal s_ref_27   : std_logic;
    signal s_ref_13_5 : std_logic := '0';
  begin
    s_ref_27 <= i_mcu_ref_clk;
    ...
    -- Decoder inputs disconnected: drive black pixels into the pipeline,
    -- gate active video by the free-running internal sync generator.
    ...
    o_mcu_gpout_clk <= '0';   -- output driver disabled at SB_IO level
    i_spi_sdo <= '0';
  end generate;

  GEN_HD_STANDALONE_IN : if C_ENABLE_HD and C_ENABLE_STANDALONE generate
    -- HD standalone uses a single PLL to synthesize the 74.25 MHz pixel
    -- clock from the 27 MHz reference. Dropframe rates (74.25/1.001 MHz)
    -- are NOT supported in standalone: firmware filters those timing IDs
    -- from the user-selectable list when a standalone bitstream is loaded.
    signal s_ref_27 : std_logic;
  begin
    s_ref_27 <= i_mcu_ref_clk;

    pll_inst : entity work.standalone_hd_video_clk_pll
    port map(
      i_clk    => s_ref_27,
      o_clk    => vid_clk,
      o_locked => open,
  ...
  GEN_DECIMATED_PROG_CLK : if C_ENABLE_HD and (C_HD_CLOCK_DIVISOR > 1) generate
    ...
    -- prog_clk is a phase-aligned integer division of vid_clk, so a single
    -- register provides clean capture. The program sees one pixel per
    -- prog_clk cycle (every Nth input pixel).
    ...
    -- Program output changes once per prog_clk cycle and is stable for N
    -- vid_clk cycles. Each output pixel is held/repeated N times, producing
    -- pixel repetition at the full output rate.
  ...
  -- ========================================================================
  -- The video sync generator's HSYNC/VSYNC reference is driven by the
  -- external source's sync pins (set in each GEN_*_IN block above) for
  -- HDMI and analog modes. In dual mode, vid_clk is the analog decoder's
  -- clock domain but the desired reference is the HDMI receiver, so a
  -- 2FF synchronizer provides clean capture across the (genlock'd but
  -- nominally separate) clock domains.
