...
-- core_config_pkg is currently unused in this file but is retained so
-- per-bitstream conditional logic can be added here in the future.
...
entity hardware_top is
  port(
    ...
    -- Pin 128: driven by the FPGA in all bitstream variants as a debug
    -- AVID heartbeat to the MCU. In earlier revisions this pin was
    -- bidirectional and used as a 27 MHz reference input in standalone
    -- bitstreams; standalone now derives its reference from the ADV7181C
    -- LLC clock instead, so the pin is a simple output everywhere.
...
begin
  ...
  -- ========================================================================
  -- Pin 128 (RP2040_GPOUT_CLK) — AVID debug heartbeat output
  -- ========================================================================
  -- Driven in all bitstream variants (including standalone) so the MCU
  -- can monitor the FPGA's active-video signal. Standalone bitstreams
  -- now use the ADV7181C LLC clock for their pixel-clock reference, so
  -- this pin is no longer bidirectional.
  RP2040_GPOUT_CLK <= s_gpout_drive;
