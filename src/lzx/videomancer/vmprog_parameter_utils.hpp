...
namespace lzx {
     ...
     */
    constexpr uint16_t apply_parameter_control_curve(uint16_t value, vmprog_parameter_control_mode_v1_0 mode)
    {
        constexpr uint16_t max_val = 1023;  // 10-bit maximum
        const uint32_t t = value > max_val ? max_val : value;  // Clamp input to valid range

        switch (mode)
        {
            // ===== Linear Scaling Modes =====
        case vmprog_parameter_control_mode_v1_0::linear:
            return value;  // 1:1 passthrough

        case vmprog_parameter_control_mode_v1_0::linear_half:
            return static_cast<uint16_t>(value >> 1);  // 0.5x scaling (optimized: shift instead of div)

        case vmprog_parameter_control_mode_v1_0::linear_quarter:
            return static_cast<uint16_t>(value >> 2);  // 0.25x scaling (optimized: shift instead of div)

        case vmprog_parameter_control_mode_v1_0::linear_double:
            return clamp_u16(value << 1, 0, max_val);  // 2x scaling (optimized: shift instead of mul)

            // ===== Boolean Mode =====
        case vmprog_parameter_control_mode_v1_0::boolean:
            return (value >= 512) ? max_val : 0;  // On/off threshold at midpoint

            // ===== Discrete Step Modes =====
        case vmprog_parameter_control_mode_v1_0::steps_4:
            return static_cast<uint16_t>((value >> 8) * 341);  // 4 steps: 0, 341, 682, 1023 (optimized)

        case vmprog_parameter_control_mode_v1_0::steps_8:
            return static_cast<uint16_t>((value >> 7) * 146);  // 8 steps: ~146 per step (optimized)

        case vmprog_parameter_control_mode_v1_0::steps_16:
            return static_cast<uint16_t>((value >> 6) * 68);  // 16 steps: ~68 per step (optimized)

        case vmprog_parameter_control_mode_v1_0::steps_32:
            return static_cast<uint16_t>((value >> 5) * 33);  // 32 steps: ~33 per step (optimized)

        case vmprog_parameter_control_mode_v1_0::steps_64:
            return static_cast<uint16_t>((value >> 4) << 4);  // 64 steps: 16 per step (optimized: mask bits)

        case vmprog_parameter_control_mode_v1_0::steps_128:
            return static_cast<uint16_t>((value >> 3) << 3);  // 128 steps: 8 per step (optimized: mask bits)

        case vmprog_parameter_control_mode_v1_0::steps_256:
            return static_cast<uint16_t>((value >> 2) << 2);  // 256 steps: 4 per step (optimized: mask bits)

            // ===== Polar/Angular Modes (with 360° wrapping) =====
        case vmprog_parameter_control_mode_v1_0::polar_degs_90:
            return static_cast<uint16_t>(t >> 2);  // 0.25 rotations (optimized: 90/360 = 1/4)

        case vmprog_parameter_control_mode_v1_0::polar_degs_180:
            return static_cast<uint16_t>(t >> 1);  // 0.5 rotations (optimized: 180/360 = 1/2)

        case vmprog_parameter_control_mode_v1_0::polar_degs_360:
            return static_cast<uint16_t>(t);  // 1 rotation (optimized: 360/360 = 1)

        case vmprog_parameter_control_mode_v1_0::polar_degs_720:
            return static_cast<uint16_t>((t << 1) & 1023);  // 2 rotations with wrap (optimized: 720/360 = 2)

        case vmprog_parameter_control_mode_v1_0::polar_degs_1440:
            return static_cast<uint16_t>((t << 2) & 1023);  // 4 rotations with wrap (optimized: 1440/360 = 4)

        case vmprog_parameter_control_mode_v1_0::polar_degs_2880:
            return static_cast<uint16_t>((t << 3) & 1023);  // 8 rotations with wrap (optimized: 2880/360 = 8)
        ...
        default:
            return value;  // Unknown mode: return input unchanged
        }
    }
     ...
     */
    constexpr uint16_t apply_parameter_control_curve_and_scaling(uint16_t value, const vmprog_parameter_config_v1_0& mode)
    {
     ...
     */
    inline constexpr uint32_t DIVISOR_LOOKUP[7] = {
    ...
    constexpr void generate_parameter_value_display_string(uint16_t value, const vmprog_parameter_config_v1_0& config, char* out_str, size_t out_str_size)
    {
        ...
        const uint32_t divisor = (config.display_float_digits < 7) ? DIVISOR_LOOKUP[config.display_float_digits] : 1000000;
